...
def objective(envs, capture_video=False, write_logs_to_file=True, seed=10, num_envs=1, parallel_envs=False):
  ...
  def objective(trial):
    ...
    Config.only_use_top_50_adv = selector("only_use_top_50_adv")
    Config.importance_sampling = selector("importance_sampling")
    Config.min_lagrange = selector("min_lagrange")
    Config.eta_initial = selector("eta_initial")
    Config.alpha_initial = selector("alpha_initial")
    Config.alpha_mu_initial = selector("alpha_mu_initial")
    Config.alpha_sigma_initial = selector("alpha_sigma_initial")
    Config.eps_alpha = selector("eps_alpha")
    Config.eps_alpha_mu = selector("eps_alpha_mu")
    Config.eps_alpha_sigma = selector("eps_alpha_sigma")
    Config.eps_eta = selector("eps_eta")
